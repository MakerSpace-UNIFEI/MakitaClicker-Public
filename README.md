...
> [!NOTE]
> **Snapshot Histórico Funcional ("As-Is"):**  
> Este repositório público preserva o código em sua versão **"as-is"**, representando fielmente o sistema na época em que o console físico estava **ativo e em pleno funcionamento** no laboratório **MakerSpace UNIFEI**.  
> Todas as funcionalidades de integração entre hardware e nuvem — como leitura instantânea do botão mecânico via interrupção (0 ms), sincronização bidirecional de telemetria, posse temporária do hardware via Web com exibição em tempo real no display LCD 20×4 e auto-update OTA — estão preservadas no código exatamente como operavam.  
> **Segurança Open-Source:** As únicas alterações feitas em relação ao código que rodou em produção foram a sanitização de segredos privados (credenciais de Wi-Fi, senhas administrativas e identificadores de infraestrutura), que foram substituídos por placeholders seguros e suporte a variáveis de ambiente.  
> O console físico foi fruto de um projeto de tempo livre dos autores; após o ciclo de demonstração, o hardware físico foi desmontado e seus componentes reaproveitados em outros projetos do MakerSpace.

---

## 📖 Visão Geral do Projeto

O **MakitaClicker** é um jogo incremental (*cookie clicker*) híbrido físico-digital desenvolvido para o MakerSpace UNIFEI. O objetivo do jogo é acumular "Makitas" até atingir a grande meta cósmica de **99 Bilhões (99B)**. 

Este repositório documenta a integração completa entre hardware e web em seu estado funcional:
1. **Console Físico Autônomo:** Um microcontrolador **ESP8266 NodeMCU** com botão mecânico industrial de alta durabilidade (0 ms de latência) e um display **LCD 20×4 I2C**. Salva o progresso na memória flash interna (**LittleFS**), sincroniza pela internet via Wi-Fi e exibe em tempo real o **Top Player** ou o **Dono Temporário** no display com suporte a posse exclusiva via Web.
...
2. **Sistema de Perfis & Salvamento na Nuvem:**
   ...
   - **Salvamento Automático & Manual no D1 (com KV Backup):** O progresso local é salvo frequentemente no Cloudflare D1 (SQL relacional com cota de 100k writes/dia) a cada 15 segundos ou sob demanda via debouncing (2-3s em compras e 5s em cliques), e replicado no Cloudflare KV como backup secundário (throttled a cada 60s), ou instantaneamente pelo botão **"💾 Salvar na Nuvem"**.
